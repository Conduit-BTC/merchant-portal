...
interface ShippingOptionState {
  ...
  createShippingOption: (
    shippingOption: Omit<ShippingOption, 'created_at'>
  ) => Promise<void>
...
export const useShippingOptionStore = create<ShippingOptionState>(
  (set, get) => ({
    ...
    fetchShippingOptions: async () => {
      ...
      try {
        ...
        const subscription = ndk.subscribe({
          ...
          authors: [(await ndk.signer!.user()).pubkey]
        })
        ...
        subscription.on('event', (event: NDKEvent) => {
          try {
            ...
            try {
              const validatedShippingOptionZodObj =
                validateShippingOption(shippingOption)
              if (!validatedShippingOptionZodObj.success)
                throw new Error(
                  'Invalid ShippingOption data from fetched Event: ' +
                    JSON.stringify(validatedShippingOptionZodObj.error)
                )

              // Add to store
              set((state) => ({
                shippingOptions: new Map(state.shippingOptions).set(
                  shippingOptionId,
                  { ...validatedShippingOptionZodObj.data, eventId }
                )
              }))
        ...
        set({
          ...
          error:
            error instanceof Error
              ? error.message
              : 'Failed to create shippingOption'
        })
      }
    },
    ...
    updateShippingOption: async (id, updatedData) => {
      ...
      try {
        ...
        // Validate with schema
        const validatedShippingOptionZodObj = validateShippingOption(
          updatedShippingOption
        )
        if (!validatedShippingOptionZodObj.success)
          throw new Error(
            'Issue updating ShippingOption: ' +
              JSON.stringify(validatedShippingOptionZodObj.error)
          )
        ...
        set({
          ...
          error:
            error instanceof Error
              ? error.message
              : 'Failed to update ShippingOption'
        })
      }
    },
      ...
      try {
        ...
        set({
          ...
          error:
            error instanceof Error
              ? error.message
              : 'Failed to delete ShippingOption'
        })
      }
    }
  })
